...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleJoinRoom(
    ...
    try {
      ...
      console.log(`User joined room ${data.roomId}, sent ${history.length} messages`);
    } catch (error) {
  ...
  async handleCreateRoom(
    client: ChatSocket,
    @MessageBody() data: { name: string; description: string | null; creatorId: number; memberEmails: string[] },
  ) {
    try {
      // Create the room
