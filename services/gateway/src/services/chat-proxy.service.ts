...
export type ChatType = 'messages' | 'users' | 'general' | 'rooms' | 'leave' | 'members';
...
export class ChatProxyService {
  ...
  getChat(type: ChatType, payload?: unknown) {
    return this.request<ResultObjectDto<unknown>>('get_chat', { type, payload });
  }

  sendChatEvent(event: string, payload: unknown) {
    return this.request<ResultObjectDto<unknown>>('chat_event', { event, payload });
  }
