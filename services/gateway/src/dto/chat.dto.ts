...
export class UserRoomsRequestDto {
  @ApiProperty({ example: 123, description: 'User ID extracted from JWT token' })
  userId: number;
}
...
export class CreateRoomResponseDto {
  @ApiProperty({ example: { id: 1, name: 'General Discussion', description: 'General chat room' }, description: 'Created room info' })
  room: RoomDto;
}
...
export class RoomMemberDto {
  ...
  @ApiProperty({ example: 1, description: 'User role in room (0=banned, 1=member, 2=moderator)' })
  role: number;

  @ApiProperty({ example: '2024-01-15T10:30:00Z', description: 'When user joined room' })
  joinedAt: string;
}
