...
import { RoomDto, UserRoomsRequestDto, LeaveRoomRequestDto, LeaveRoomClientRequestDto, CreateRoomRequestDto, CreateRoomResponseDto, GetRoomMembersRequestDto, RoomMemberDto } from '../dto/chat.dto';
...
export class ChatController {
  ...
  async getRoomMembers(@Body() body: GetRoomMembersRequestDto) {
    ...
    const chatResponse = await this.chatProxyService.getChat('members', { roomId: body.roomId });
    
    if (chatResponse.error) {
      return chatResponse;
    }

    const roomMembers = chatResponse.object as Array<{ userId: number; role: number; joinedAt: string }>;
    ...
    const users = usersResponse.object as Array<{ id: number; email: string; firstName: string | null; lastName: string | null }>;
    ...
    return { error: false, htmlcode: 200, object: enrichedMembers, messages: [] };
  }

  @HttpCode(200)
  @Post('leave')
  @ApiOperation({ summary: 'Leave chat room' })
  @ApiBody({ type: LeaveRoomClientRequestDto, description: 'Leave room request (userId extracted from JWT)' })
  @ApiOkResponse({
    type: ResultObjectDto<null>,
    description: 'Leave room response',
  })
  leaveChat(@Body() body: LeaveRoomClientRequestDto, @CurrentUser() user: AuthenticatedUser) {
